...
const breadcrumbItems = [
  ...
  { label: "Modification informations cours", isCurrent: true },
];
...
const EditCourse = () => {
  ...
  const { data: course, error } = useFetchData<Course>(
    `/api/courses/${id}`
  );
  ...
  useEffect(() => {
    if (course) {
      setFormData({
        ...
        department_id: course.department_id || 0,
      });
    }
  }, [course]);
  ...
  if (error || errorLoadDepartment) return <ErrorCard message={typeof error !== "undefined" ? error?.message : ""} />;
